<?php
    ...
    echo '<h3>Terminarz dla: '.$team_name[1].'</h3><br><br><hr>
        ...
        <button>Dodaj kolejkę</button></a><br><br>';
    ...
    while($row = mysqli_fetch_array($queues)) {
        ...
        echo '<div class="row"><h4>Kolejka nr: '.$row[2].'</h4><br>
            <table>
                <tr><th>Gospodarze:</th><th>Wynik:</th><th>Goście:</th><th>Data:</th><th>Godzina:</th><tr>';
        while($s = mysqli_fetch_assoc($schedule)) {
            echo '<tr><form action="/promnik/admin/panel/model/edit_task.php?id='.$s['schedule_id'].'" method="post">
                    ...
                    <td><input type="submit" value="Zapisz"></form>
                    <a href="/promnik/admin/panel/model/del_task.php?id='.$s['schedule_id'].'">
