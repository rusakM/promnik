<?php
    ...
    echo '<br><h3>Terminarz dla: '.$team_name[1].'</h3><br><hr>
        <a href="/promnik/admin/panel/model/add_queue.php?team='.$team.'">
        <button class="btn">Dodaj kolejkę</button></a>';
    if(mysqli_num_rows(mysqli_query($con, "SELECT * FROM queues WHERE team_id='$team'")) > 0) {
        echo '<a href="/promnik/admin/panel.php?strona=dodajmecz&team='.$team.'">
            <button class="btn">Dodaj mecz</button></a>'; 
    }
    echo '<br><br>';
    ...
    while($row = mysqli_fetch_array($queues)) {
        ...
        $tasks = mysqli_num_rows($schedule);
            echo '<div class="form-control"><h4>Kolejka nr: '.$row[2].'</h4> (<a href="/promnik/admin/panel/model/del_q.php?id='.$q_id.'&team='.$team.'">Usuń</a>)<br>';
        if($tasks > 0) {
            ...
            while($s = mysqli_fetch_assoc($schedule)) {
                echo '<tr><form action="/promnik/admin/panel/model/edit_task.php?id='.$s['schedule_id'].'&team='.$team.'" method="post" class="row">
                        <td><input type="text" name="hosts" value="'.$s['hosts'].'"></td>
                        <td><input type="number" name="score_h" class="col-5" value="'.$s['score_hosts'].'"> : 
                        <input type="number" name="score_g" class="col-5" value="'.$s['score_guests'].'"></td>
                        ...
                        <td><input type="submit" value="Zapisz" class="btn btn-default"></td></form><td>
                        <a href="/promnik/admin/panel/model/del_task.php?id='.$s['schedule_id'].'&team='.$team.'">
                        <button class="btn btn-default">Usuń</button></a></td></tr>';
